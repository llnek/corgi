...
(ns ^{:doc ""
  ...
  (:require [czlab.mcfud.p2d.core :as pc :refer [gWorld]]
            [czlab.mcfud.afx.math :as m :refer [EPSILON V2]]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn set-orient!
  "Set angle of the body in radians."
  [B radians]
  (let [{{:keys [set-angle] :as S} :shape} @B]
    (assert (fn? set-angle) "set-angle missing")
    (c/assoc!! B
               :angle radians :shape (set-angle S radians))))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- set-pos-circle [C pt] (c/assoc!! C :pos pt))
...
(defn circle
  ...
  ([r options]
   (pc/set-body-attrs!
     (-> (assoc (g/circle r)
                :set-angle set-circle-angle)
         (pc/body {:repos set-pos-circle})) options)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- set-poly-angle [P radians]
  (assoc P :u (m/rotation2x2 radians)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- draw-poly [P ctx]
  (let [{c :pos
         {:keys [u vertices]} :shape} @P]
    (gx/draw-poly*
      (mapv #(m/vec-add c
                        (m/mat-vmult u %)) vertices) ctx) P))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- set-pos-poly [P pt] (c/assoc!! P :pos pt))
...
(defn- polygon* []
  (pc/body (assoc (g/polygon [])
                  :normals []
                  :u (m/rotation2x2 0)
                  :set-angle set-poly-angle)
           {:repos set-pos-poly}))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn polygon-box
  "Vertices are ordered counter-clockwise, starting from
  lower left bottom.
            |
            n1
      V2____|____V1
     ...
     (c/assoc!! P
                ...
                (assoc shape
                       :vertices [(V2 hw (- hh))
                                  ...
                                  (V2 (- hw) hh)
                                  (V2 (- hw) (- hh))]
                       :normals [(V2 1 0) (V2 0 1)
                                 (V2 -1 0)(V2 0 -1)]))
    (set-orient! P 0)
    (pc/set-body-attrs! P options))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- calc-face-normals! [P]
  (let [{{:keys [vertices] :as S} :shape} @P
        EE (c/sqr* EPSILON)]
    ;;counter-clockwise so normals
    ;;point out from edge to world
    (loop [i 0 SZ (n# vertices) out (c/tvec*)]
      (if (>= i SZ)
        (c/assoc!! P
                   :shape
                   (assoc S :normals (c/ps! out)))
        (let [i2 (m/wrap?? i SZ)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- impulse-body! [B impulse contact]
  (let [{:keys [vel im ii gvel]} @B]
    (c/assoc!! B
               :vel (m/vec-add vel (m/vec-scale impulse im))
               :gvel (+ gvel (* ii (m/vec-xss contact impulse))))))
...
(defn- init-manifold! [M]
  (let [{:keys [gravity frame-secs]} @gWorld
        {:keys [contacts A B]} @M
        {gvelA :gvel velA :vel posA :pos
         bounceA :bounce dfA :dynaF sfA :statF} @A
        {gvelB :gvel velB :vel posB :pos
         bounceB :bounce dfB :dynaF sfB :statF} @B]
    (c/assoc!! M
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- impulse-manifold! [M]
  (let [{:keys [A B bounce contacts
                normal statF dynaF]} @M]
    (loop [i 0 SZ (n# contacts) loop? true]
      (when (and loop? (< i SZ))
        (let [{imA :im iiA :ii
               gvelA :gvel velA :vel posA :pos} @A
              {imB :im iiB :ii
               gvelB :gvel velB :vel posB :pos} @B]
          (recur (+ 1 i) SZ
                 (let [c (nth contacts i)
                       ;;calculate radii from COM to contact
                       ra (m/vec-sub c posA)
                   ...
                   (if (pos? contactVel)
                     false
                     (let [raXN (m/vec-xss ra normal)
                           rbXN (m/vec-xss rb normal)
                           invMass (+ imA imB
                                      (* (c/sqr* raXN) iiA)
                                      (* (c/sqr* rbXN) iiB))
                       ...
                       ;;Apply impulse
                       (impulse-body! A (m/vec-neg impulse) ra)
                       (impulse-body! B impulse rb)
                       ;;Friction impulse
                       (let [{gvelA :gvel velA :vel} @A
                         ...
                         (if (m/fuzzy-zero? jT)
                           ...
                           (let [tangentImpulse
                             ...
                             (impulse-body! A (m/vec-neg tangentImpulse) ra)
                             (impulse-body! B tangentImpulse rb) true))))))))))))
...
(defn- positional-correction! [M]
  (let [{:keys [A B normal penetration]} @M
    ...
    (c/assoc!! A
               :pos (m/vec-sub posA
                               (m/vec-scale jiggle imA)))
    (c/assoc!! B
               :pos (m/vec-add posB
                               (m/vec-scale jiggle imB))) M))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn apply-force "Apply force to the body." [B f]
  (swap! B
         (fn [{:keys [accel] :as root}]
...
(defn- circle->circle [M B1 B2]
  (let [{posA :pos {radA :radius :as A} :shape} @B1
        {posB :pos {radB :radius :as B} :shape} @B2
        radius (+ radA radB)
    ...
    (cond
      ...
      (let [dist (c/sqrt* distSQ)]
        (if (zero? dist)
          (c/assoc!! M
                     :penetration radA
                     :normal (V2 1 0) :contacts [posA])
          (swap! M
                 #(let [n (m/vec-scale normal (/ 1 dist))]
                    (assoc %
                           :penetration (- radius dist)
                           :normal n
                           :contacts [(m/vec-add (m/vec-scale n radA) posA)])))))) M))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- circle->polygon*
  [M B1 B2 center separation faceNormal]
      ...
      (if (or break? (>= i SZ))
        ...
        (let [s (m/vec-dot (nth normals i)
              ...
              (cond (> s radA) [sep n true]
                    (> s sep) [s i break?]
                    :else [sep n break?])] (recur (+ 1 i) SZ s' n' t')))) M))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- axis-least-penetration [A B]
  ...
  (loop [i 0 SZ (n# vertices) bestD NEG-INF bestIndex 0]
    (if (>= i SZ)
      ...
      (let [nw (m/mat-vmult au (nth normals i))
            ...
            b? (> d bestD)]
        (recur (+ 1 i) SZ (if b? d bestD) (if b? i bestIndex)))))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn- incident-face?? [refPoly incPoly refIndex]
  (let [{bI :body iu :u
         :keys [normals vertices]} incPoly
        {posI :pos} @bI
    ...
    (loop [i 0 SZ (n# vertices)
      ...
      (if (>= i SZ)
        ...
        (let [dot (m/vec-dot refNormal (nth normals i))
              b? (< dot minDot)]
          (recur (+ 1 i) SZ (if b? i iFace) (if b? dot minDot)))))))
...
(defn- polygon->polygon* [M B1 B2
                          [penetrationA faceA]
                          [penetrationB faceB]]
  (let [[A B] [(:shape @B1) (:shape @B2)]
        ;flip Always point from a to b
        [refPoly incPoly refIndex flip?]
        ;;Determine which shape contains reference face
        (if (m/bias-greater? penetrationA
                             penetrationB)
          [A B faceA false] [B A faceB true])
        [bR bI] [(:body refPoly) (:body incPoly)]
        {posR :pos} @bR
        ;;World space incident face
        incidentFaces (incident-face?? refPoly incPoly refIndex)
        ...
        ;;Orthogonalize
        refFaceNormal (V2 (_2 sidePlaneNormal)
                          (- (_1 sidePlaneNormal)))
        ;;ax + by = c
    ...
    (when-not (or quit1? quit2?)
      ...
      (let [sep (- (m/vec-dot refFaceNormal
                              (_1 incidentFaces)) refC)]
        (if (<= sep 0)
          (c/assoc!! M
                     :penetration (- sep)
...
(defn- polygon->polygon [M B1 B2]
  ...
  (let [[A B] [(:shape @B1) (:shape @B2)]
        ...
        [penetrationA faceA] (axis-least-penetration A B)
        skipA? (>= penetrationA 0)
        ;;Check for a separating axis with B's face planes
        [penetrationB faceB] (if skipA?
                               [1 1] ;; hack to skip
                               (axis-least-penetration B A))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(def ^:private vtbl {:polygon draw-poly :circle draw-circle})

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn draw-body "" [B ctx]
  (c/if-func
    [f (vtbl (get-in @B
                     [:shape :type]))]
    (f B ctx)
    (c/raise! "Bad shape to draw!")))
...
(defn init-physics
  ...
  ([gravity fps world options]
   (pc/init gravity
            fps
            world
            (merge {:algo-runner run-algo} options))))
